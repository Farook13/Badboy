...
async def give_filter(client, message):
    if message.chat.id != SUPPORT_CHAT_ID:
        ...
        if total_results == 0:
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔍 ᴊᴏɪɴ ᴀɴᴅ ꜱᴇᴀʀᴄʜ ʜᴇʀᴇ 🔎", url=f"https://t.me/TmaxCinema")]]))
...
async def pm_text(bot, message):
    ...
    await message.reply_text(
         ...
         reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ", url=f"https://t.me/TmaxCinema")]])
    )
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("🔍 ꜱᴇᴀʀᴄʜ ʜᴇʀᴇ 🔎", url="https://t.me/TmaxCinema")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        try:
            ...
            if await db.has_premium_access(user_id):                               
                buttons = [[
                    ...
                    InlineKeyboardButton('📌 ᴊᴏɪɴ ᴜᴘᴅᴀᴛᴇꜱ ᴄʜᴀɴɴᴇʟ 📌', url='https://t.me/+JeWFhkIdXkdiNzc1')
                ]]
            else:
    ...
    # Thanks to @CoderluffyTG for fixing this 
        ...
        buttons = [[
            InlineKeyboardButton('⚜️ ᴜᴘᴅᴀᴛᴇꜱ ᴄʜᴀɴɴᴇʟ ⚜️', url='https://t.me/+JeWFhkIdXkdiNzc1')
        ],[
            InlineKeyboardButton('⚡ʀᴇq ᴍᴏᴠɪᴇꜱ⚡', url='https://t.me/TmaxCinema'),
            InlineKeyboardButton('✨️ᴏᴛᴛ ᴍᴏᴠɪᴇꜱ✨️', url='https://t.me/+JeWFhkIdXkdiNzc1')
        ],[
            InlineKeyboardButton('🎬 𝐌𝐨𝐯𝐢𝐞𝐬 ʀᴇq 🎬', url='https://t.me/TmaxCinema')
        ],[
            InlineKeyboardButton('⇇ ʙᴀᴄᴋ', callback_data='start'),
            InlineKeyboardButton('ꜱᴜᴘᴘᴏʀᴛ 👮', url='https://t.me/Lizamwol_filterbot')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            search = message.text
            m=await message.reply_sticker("CAACAgEAAxkBAAIZAmYcCEHOgUvX5W1MMkST-5VyNVfJAALRAgAC3sXYRzU-aLmaObA3NAQ",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f'Seaching for {search} 🔎', url=f"https://t.me/TmaxCinema")]]) 
            )
            search = search.lower()
            find = search.split(" ")
            search = ""
        ...
        m=await message.reply_sticker("CAACAgEAAxkBAAIZAAFmHAg7n2fFR8Zg1klRvS8JXkFRTAACYgIAAhbl2UfX7tmBE7IAAdo0BA",                              
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f'Seaching for {search} 🔎', url=f"https://t.me/TmaxCinema")]]) 
        )
        settings = await get_settings(message.chat.id)
        await m.delete()
...
async def advantage_spell_chok(client, msg):
    ...
    try:
        ...
        if not movielist:
            reqst_gle = query.replace(" ", "+")
            button = [[
                       InlineKeyboardButton("📝 ʀᴇǫᴜᴇꜱᴛ ʜᴇʀᴇ", url=f"https://t.me/TmaxCinema")
            ]]
            if NO_RESULTS_MSG:
                await client.send_message(chat_id=LOG_CHANNEL, text=(script.NORSLTS.format(reqstr.id, reqstr.mention, mv_rqst)))
            k = await msg.reply_text(
